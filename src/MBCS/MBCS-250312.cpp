...
// else {
        ...
        while (sz[tt] <= g->mxd[zz] && sz[zz] >= crsz[zz]) {
            ...
            if (alg == 4){sz[t] = crsz[t]; sz[z] = crsz[z];}
...
// }
    ...
    if (mSize / crsz[zz] <= crsz[tt] || alg == 4) {
...
void MBCS::run_basic() {
    ...
    #ifdef UB1_
    printf("run: defined_UB1\n");
    #else
    printf("run: undefined_UB1\n");
    #endif
    // for(uint32_t i = 0; i < g->n[1]; ++i) {
    //     uint32_t d = g->p[1][i+1] - g->p[1][i];
    //     if (d >= 7591)
    ...
    // if (sz[tt] * sz[zz] > mSize) { sz[zz] = std::max(crsz[zz], uint32_t(mSize / sz[tt] / stp)); }

    stp = 2;
    uint32_t it = 0;
    unsigned long maxBranchduration = 0;
    for (uint32_t u, ui = 0; ui < g->n[t]-1; ui++) {
        u = ui;
        S[t].r = S[t].c = 0;
        S[z].r = S[z].c = 0;
        if (S[t].capacity() <= u) {
...
// }
    ...
    //     if (it > 0) {
    //         if (uint32_t(sz[zz] / stp) < crsz[zz]) {
    //             sz[tt] = uint32_t(sz[tt] * sz[zz]/crsz[zz]);
    //             sz[zz] = crsz[zz];
    //         }
    //         else {
    //             sz[tt] = std::max(sz[tt]+1, uint32_t(sz[tt] * stp));
    //             sz[zz] = uint32_t(mSize / sz[tt] / stp); 
    //         }
    //     }
    ...
    //     for (uint32_t ttt = 0; ttt <= 1; ++ttt) {
    //         uint32_t zzz = ttt^1;
    //         for (uint32_t u = 0; u < g->n[ttt]; ++u){
    //             sdeg[ttt][u] = g->p[ttt][u+1]-g->p[ttt][u];
    //             if (sdeg[ttt][u] < sz[zzz]) {
    //                 vqueue[ttt][qsz[ttt]++] = u;
    //                 sdeg[ttt][u] = 0;
    //             }
    //         }
    //     }
    ...
    //     std::vector<uint32_t> comnbr;
    //     comnbr.resize(std::max(g->n[0], g->n[1]), 0);

    //     for (uint32_t ttt = 0; ttt <= 1; ++ttt) {
    //         uint32_t zzz = ttt^1;
    //         test_v[ttt] = 0;
    //         for (uint32_t u = 0; u < g->n[ttt]; ++u){
    //             if (sdeg[ttt][u] < sz[zzz]) test_v[ttt]++;
    //             // test_v[ttt]++;
    //         }
    //     }
    //     // if (test_v[0] != qsz[0]) {
    //         // printf("test_v[0]=%d, qsz[0]=%d, qr[0]=%d, test_v[1]=%d, qsz[1]=%d, qr[1]=%d\n", test_v[0], qsz[0], qr[0], test_v[1], qsz[1], qr[1]);
    //     // }
    //     assert(test_v[0] == qsz[0]);
    //     assert(test_v[1] == qsz[1]);

    //     for (uint32_t ttt = t; ttt <= 1; ++ttt) {
    //         uint32_t zzz = ttt^1;
    //         for (uint32_t u = 0; u < g->n[ttt] && qsz[ttt] + sz[ttt] <= g->n[ttt]; ++u){
    //             if (sdeg[ttt][u] < sz[zzz]) continue;
    //             S[ttt].c = 0;
    //             // printf("u=%d, S[t].c=%d\n", u, S[ttt].c);
    //             for (uint32_t i = g->p[ttt][u]; i < g->p[ttt][u+1]; ++i) {
    //                 uint32_t v = g->e[ttt][i];
    //                 if (sdeg[zzz][v] < sz[ttt]) continue;
    //                 for (uint32_t j = g->p[zzz][v]; j < g->p[zzz][v+1]; ++j) {
    //                     uint32_t w = g->e[zzz][j];
    //                     uint32_t dw = sdeg[ttt][w];
    //                     if (dw >= sz[zzz]) {
    //                         uint32_t wp = S[ttt].pos(w);
    //                         if (wp >= S[ttt].c) S[ttt].swapByPos(S[ttt].c++, wp);
    //                         comnbr[w]++;
    //                     }
    //                 } 
    //             }
    //             // printf("u=%d, S[t].c=%d\n", u, S[ttt].c);
    //             uint32_t cnts = 0;
    //             assert(S[ttt].c <= g->n[ttt] - qsz[ttt]);
    //             for (uint32_t j = 0; j < S[ttt].c; ++j) {
    //                 uint32_t v = S[ttt][j];
    //                 if (comnbr[v] >= sz[zzz]) cnts++;
    //                 comnbr[v] = 0;
    //             }
    //             // printf("u=%d, S[t].c=%d, cnts=%d\n", u, S[ttt].c, cnts);
    //             if (cnts < sz[ttt]) {
    //                 sdeg[ttt][u] = 0;
    //                 qsz[ttt]++;
    //             }
    ...
    //     }
    //     // printf("test111\n");
    //     if (g->n[t]-qsz[t] >= sz[t]  && g->n[z]-qsz[z] >= sz[z]) {
    ...
    //             for (uint32_t u = 0; u < g->n[ttt]; ++u){
    //                 if (sdeg[ttt][u] >= sz[zzz]) {
    //                     S[ttt].swapByPos(S[ttt].c++, S[ttt].pos(u));
    //                 }
    //             }
    //         } 
    //         printf("S[t].c=%d, S[z].c=%d\n", S[t].c, S[z].c);
    //         lrs[t] = S[t].c; lrs[z] = S[z].c; Sc[t].c = 0; Sc[z].c = 0;
    //         branch_basic(0);
    //         // branchNew(0,0);
...
void MBCS::run_basic1() {
    ...
    // if (mSize / crsz[zz] <= crsz[tt]) {
        auto t22 = std::chrono::steady_clock::now();
        auto duration1 = std::chrono::duration_cast<std::chrono::milliseconds>(t22 - t1);
        printf("mSize=%d, lsz=%d,rsz=%d, mxdeep=%d, numbranches=%ld, maxsubbranches=%ld\n", mSize, realsz[0], realsz[1], mxdeep, numbranches, maxsubbranches);
        std::cout << "iteration time:" << duration1.count() << "ms" << std::endl;
        printRes();
        return;  
    // }
...
void MBCS::branch_basic(uint32_t deep) {
    ...
    uint32_t qsz[2] = {0};
    uint32_t t = 0, z = 1;
    if (g->mxd[t] < g->mxd[z]) {t = z; z = t^1;}
    if (S[t].cSize() > 10*S[z].cSize()) {t = z; z = t^1;}
    ...
    int minE = g->n[z]+1, minU = S[t][S[t].r];
    ...
    for (uint32_t i = S[t].r; i < S[t].c; ++i) {
        ...
        bool is_comnbr = true;
        for(int j = S[z].r; j < S[z].c; j++) {
            if(!g->is_nbr(t, v, S[z][j])) {
                is_comnbr =false;
                break;
            }
        }
        if (is_comnbr) S[t].swapByPos(S[t].r++, i);
    }

    bool is_comnbr = true;
    // uint32_t stsz = Sc[t].c;
    // for (uint32_t i = 0; i < Sc[t].c && is_comnbr; ++i) {
    //     uint32_t v = Sc[t][i];
    //     uint32_t j = 0;
    //     for(; j < S[z].r; j++) {
    //         if(!g->is_nbr(t, v, S[z][j])) {
    //             Sc[t].swapByPos(i, --Sc[t].c); i--;
    //             break;
    //         }
    //     }
    //     for(; j >= S[z].r && j < S[z].c; j++) {
    //         if(!g->is_nbr(t, v, S[z][j])) break;
    //     }
    //     if (j == S[z].c) { is_comnbr = false; break;}
    // }
...
//         for (uint32_t i = 0; i < S[t].c; ++i){
//             uint32_t v = S[t][i];
//             uint32_t d = alg==4? sdeg[t][v] : _deg[t][v];
//             // if (d < sz[z]) {
//             //     printf("v =%d, d = %d,  _deg[t][v]=%d\n", v, d, _deg[t][v]);
//             //     printf("S[t].c=%d, S[z].c=%d, S[t][0]=%d, S[z][0]=%d\n", S[t].c, S[z].c, S[t][0], S[z][0]);
//             // }
...
//     for (uint32_t i = 0; i < S[z].c; ++i){
//         uint32_t v = S[z][i];
//         uint32_t d = alg==4? sdeg[z][v] : _deg[z][v];
//         uint32_t x = d;
//         d = S[t].c - x;
//         gs->p[z][i+1] = gs->p[z][i] + d;
//     }
